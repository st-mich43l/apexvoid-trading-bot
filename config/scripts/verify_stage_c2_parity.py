...
DIVERGENCES: dict[str, dict] = {
  ...
  "analysis.yml/analysis.technical_authority.mode": {"kind": "new_surfaced", "expected": "go_shadow", "reason": "S14F — Go analysis authority rollout gate; no old-config precedent. go_shadow runs the policy dry run only and moves no authority; Python remains the sole publisher."},
  "analysis.yml/analysis.technical_authority.consumer_enabled": {"kind": "new_surfaced", "expected": True, "reason": "S14F — starts the durable Go opportunity consumer for the go_shadow window; publication authority is unchanged (fence: Python-owned)."},
