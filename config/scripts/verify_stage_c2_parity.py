...
DIVERGENCES: dict[str, dict] = {
  ...
  "analysis.yml/analysis.technical_authority.mode": {"kind": "new_surfaced", "expected": "go", "reason": "S14G — Go analysis authority rollout gate; no old-config precedent. mode go enables the Go->policy path but grants no authority: scopes stay Python-owned until an operator-approved fenced transfer."},
  "analysis.yml/analysis.technical_authority.consumer_enabled": {"kind": "new_surfaced", "expected": True, "reason": "S14G — the durable Go opportunity consumer stays on; publication authority is unchanged until a fenced transfer (fence: Python-owned)."},
