...
// confirmedRejection is demand's independent, strict M5 close-bar
...
func confirmedRejection(z zone.Zone, candles []market.Candle) *opportunity.ReactionConfirmation {
	if len(candles) < 2 || z.Kind != zone.KindDemand || z.State != zone.StateTouched ||
		z.TouchCount < 1 || z.LastTouchedAt == nil || z.BreakIndex >= len(candles)-1 {
		return nil
	}
	current := candles[len(candles)-1]
	if current.Time <= z.CreatedAt || !current.IsBullish() || current.Close <= float64(z.High) {
		return nil
	}
	...
	if touchTime != current.Time {
		...
		if touch.IsBullish() && touch.Close > float64(z.High) {
