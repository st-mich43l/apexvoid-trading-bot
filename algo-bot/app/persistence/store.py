...
async def init_db() -> None:
  ...
  async with _connect() as db:
    ...
    # S13B technical-authority fence. One row per (symbol, catalog strategy)
    # scope; absent row == Python owns it at epoch 0. `epoch` is a monotonic
    # fencing token advanced by every handover, so a stale operator/process can
    # never overwrite a newer decision. Handover passes through 'draining'
    # (neither side may publish) before the target owner takes effect.
    await db.execute(
      """
      CREATE TABLE IF NOT EXISTS analysis_authority_scopes (
        symbol TEXT NOT NULL,
        strategy_id TEXT NOT NULL,
        owner TEXT NOT NULL CHECK (owner IN ('python', 'go', 'draining')),
        target_owner TEXT CHECK (target_owner IN ('python', 'go')),
        epoch BIGINT NOT NULL,
        drain_until BIGINT NOT NULL DEFAULT 0,
        updated_at BIGINT NOT NULL,
        updated_by TEXT NOT NULL,
        reason TEXT NOT NULL,
        evidence_ref TEXT,
        PRIMARY KEY (symbol, strategy_id),
        CHECK ((owner = 'draining') = (target_owner IS NOT NULL))
      )
      """
    )
    await db.execute(
      """
      CREATE TABLE IF NOT EXISTS analysis_authority_transitions (
    ...
    # S14B: one row per process boot (and per consumer_enabled change) so a
    # flag flip is never invisible in the authority audit trail. Append-only.
    await db.execute(
      """
      CREATE TABLE IF NOT EXISTS analysis_authority_runtime_audit (
        audit_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        at BIGINT NOT NULL,
        event TEXT NOT NULL,
        consumer_enabled BOOLEAN NOT NULL,
        mode TEXT NOT NULL,
        previous_consumer_enabled BOOLEAN,
        go_bound_scopes JSONB NOT NULL DEFAULT '[]'::jsonb,
        detail TEXT NOT NULL DEFAULT ''
      )
      """
    )
    # Operator-recorded production acceptance. Nothing in this codebase writes
    # a row here: a scope cannot move to Go without a human-approved,
    # unexpired record naming the exact evidence.
    await db.execute(
      """
      CREATE TABLE IF NOT EXISTS analysis_authority_acceptance (
        symbol TEXT NOT NULL,
        strategy_id TEXT NOT NULL,
        ...
        PRIMARY KEY (symbol, strategy_id, evidence_ref)
      )
      """
    )
