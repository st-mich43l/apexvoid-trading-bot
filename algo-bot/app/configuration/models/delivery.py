...
class DeliveryTelegramConfig(FrozenConfigModel):
    ...
    owner_dm_daily_wipe_enabled: bool = config_field(False, canonical_env='DELIVERY_OWNER_DM_DAILY_WIPE_ENABLED', owner=ConfigOwner.PYTHON, reload=ReloadPolicy.IMMEDIATE, runtime_reload=ReloadPolicy.RESTART, unit=ConfigUnit.BOOLEAN, risk=RiskClassification.DELIVERY, description="Delete the ApexVoid bot's own messages in the owner DM at each local trade-day rollover; the owner's own typed messages are never touched. Opt-in, irreversible.", default_contexts=(ContextDefault(DefaultContext.PYTHON_SCHEMA, False),), validation_summary='Pydantic required/type coercion only')
