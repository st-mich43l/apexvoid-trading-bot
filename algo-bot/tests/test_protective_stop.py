...
def test_scalp_room_synced_stop_allows_thin_targets():
  ...
  # Scalp uses the configured volume-only multiplier while its structural
  # stop remains the same room-derived 15-pip level.
  assert scalp.measured["match_risk_multiplier"] == 2.0
  assert scalp.measured["effective_risk_multiplier"] == 2.0
